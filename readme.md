...
## 擴展調校參數
  ...
  - 分割區數量 (Partition count) 決定了 tasks.max 的最大有效值，並決定了Topic的總吞吐量如何分配到各個任務 (Task) 中（使用Hash根據Message Key/ Partition Key做分配，若Partition Key具有高基數且均勻分佈，則分配會大致平均，所以應該使用獨特值的key, 如 ID, UUID）。(check details)  
